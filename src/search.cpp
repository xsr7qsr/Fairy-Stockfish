        ...
        while (   (move = mp.next_move()) != MOVE_NONE
            ...
            if (move != excludedMove && pos.legal(move))
            {
                ...
                ss->continuationHistory = &thisThread->continuationHistory[ss->inCheck]
                                                                          [captureOrPromotion]
                                                                          [pos.history_slot(pos.moved_piece(move))]
                                                                          [to_sq(move)];
...
moves_loop: // When in check, search starts from here
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
              ...
              && !(pos.must_capture() && pos.attackers_to(to_sq(move), ~us)))
          {
              ...
              if (   lmrDepth < 4 + ((ss-1)->statScore > 0 || (ss-1)->moveCount == 1)
                  && (*contHist[0])[pos.history_slot(movedPiece)][to_sq(move)] < CounterMovePruneThreshold
                  && (*contHist[1])[pos.history_slot(movedPiece)][to_sq(move)] < CounterMovePruneThreshold)
                  continue;
              ...
              if (   lmrDepth < 7
                  ...
                  &&  (*contHist[0])[pos.history_slot(movedPiece)][to_sq(move)]
                    + (*contHist[1])[pos.history_slot(movedPiece)][to_sq(move)]
                    + (*contHist[3])[pos.history_slot(movedPiece)][to_sq(move)]
                    + (*contHist[5])[pos.history_slot(movedPiece)][to_sq(move)] / 2 < 27376)
                  continue;
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    while ((move = mp.next_move()) != MOVE_NONE)
    {
      ...
      if (  !captureOrPromotion
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY
          && (*contHist[0])[pos.history_slot(pos.moved_piece(move))][to_sq(move)] < CounterMovePruneThreshold
          && (*contHist[1])[pos.history_slot(pos.moved_piece(move))][to_sq(move)] < CounterMovePruneThreshold)
          continue;
  ...
  void update_continuation_histories(const Position& pos, Stack* ss, Piece pc, Square to, int bonus) {
    ...
    for (int i : {1, 2, 4, 6})
    {
        ...
        if (is_ok((ss-i)->currentMove))
            (*(ss-i)->continuationHistory)[pos.history_slot(pc)][to] << bonus;
    }
  }
