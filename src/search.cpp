...
moves_loop: // When in check, search starts from here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
      ...
      if (    depth >= 3
          ...
          && (  !captureOrPromotion
              || moveCountPruning
              || ss->staticEval + VariantPieceValue[EG][pos.captured_piece()] <= alpha
              || cutNode
          ...
          && !(pos.must_capture() && (givesCheck || pos.has_capture())))
      {
          ...
          else
          {
              ...
              if (   !givesCheck
                  && ss->staticEval + VariantPieceValue[EG][pos.captured_piece()] + 210 * depth <= alpha)
                  r++;
          }
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    while ((move = mp.next_move()) != MOVE_NONE)
    {
          ...
          && !pos.advanced_pawn_push(move))
      {
          ...
          futilityValue = futilityBase + VariantPieceValue[EG][pos.piece_on(to_sq(move))];
